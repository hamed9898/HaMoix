...
> پیش‌نیازها: سرور اوبونتو/دبیان با دسترسی root و دامنه (HTTPS الزامی است).
> برای نصب عادی پورت‌های 80 و 443، و برای نصب جایگزین پورت 8443 یا پورت آزاد انتخابی را باز کنید.
...
اگر پورت‌های 80 و 443 توسط پنل VPN یا سرویس دیگری اشغال هستند، این دستور را اجرا کنید:
...
این نصب‌کننده همان PHP، MariaDB، Nginx، Composer، cron و Wizard را نصب می‌کند، اما فقط روی HTTPS پورت 8443 گوش می‌دهد. بعد از نصب از آدرس زیر وارد شوید:

```text
https://panel.example.com:8443/installer/
```

پورت 8443/TCP باید در فایروال باز باشد. این مسیر برای مستقل بودن از 80/443 گواهی self-signed می‌سازد؛ مرورگر در اولین ورود هشدار گواهی نمایش می‌دهد و باید ادامه‌ی ورود را تأیید کنید. پس از Wizard، می‌توانید گواهی معتبر دامنه را روی فایل Nginx جایگزین کنید. اگر UFW فعال باشد، اسکریپت آن را خودکار باز می‌کند.

برای استفاده از پورت آزاد دیگری، عدد پورت را به‌عنوان آرگومان سوم بدهید:
...
آدرس Wizard در این حالت `https://panel.example.com:9443/installer/` خواهد بود.
